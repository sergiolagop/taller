...
class Ion_auth
{
	...
	public function logged_in()
	{
		...
		if($recheck){
			$check_permiso = $this->menu_model->get_menus($user_data['user_id']);
			$newarray = array('');
			foreach ($check_permiso as $k => $v) {
				if($v['controlador'] != "")
					$newarray[] = $v['controlador'];
				if($v['metodo'] != "")
					$newarray[] = $v['metodo'];
			}
			// echo print_r($newarray,true);
			// $last_segment = $this->uri->segment(count($this->uri->segment_array()));
			$first_segment = $this->uri->segment(1);
			$last_segment = $this->uri->segment(2);
			
			// echo "$first_segment - $last_segment"; 
			if(base_url() != current_url() ){
				if(array_search($first_segment,$newarray) > 0 )
					return $recheck;
				if(array_search($last_segment,$newarray) > 0 )
					return $recheck;

				if(array_search($first_segment,$newarray) === false){
					show_error("Pagina no encontrada1", "404", $heading = 'An Error Was Encountered'); 
				}
				if(array_search($last_segment,$newarray) === false){
					show_error("Pagina no encontrada2", "404", $heading = 'An Error Was Encountered'); 
				}
			}
		}

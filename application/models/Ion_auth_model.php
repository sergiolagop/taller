...
class Ion_auth_model extends CI_Model
{
	...
	public function login($identity, $password, $remember=FALSE)
	{
		...
		$query = $this->db->select($this->identity_column . ', email, id, password, active, last_login,id_personas')
						  ->where($this->identity_column, $identity)
						  ->limit(1)
						  ->order_by('id', 'desc')
						  ->get($this->tables['users']);
		...
		if ($query->num_rows() === 1)
		{
			...
			if ($this->verify_password($password, $user->password, $identity))
			{
				...
				$q3 = $this->db->select('id_empresas')->where('id',$user->id_personas)->where('activo',1)->get('personas');
				$out =  $q3->row();
				if($out->id_empresas == null) $out->id_empresas = 0;

				$user->id_empresas = $out->id_empresas;
				// echo $this->db->last_query();
				// echo print_r($user,true); die;
